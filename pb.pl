...
### Updates all files that get sent to machines in-game
my $update = time();
foreach my $path(keys %scripts) {
    ...
    $content =~ s/^ --\s?  $bn  .* $/--$bn $hash $update/xm;           # no 's'.
...
### Update the Sphinx docs config file to show the latest version number.
my $config_path = "$FindBin::Bin/write_docs/conf.py";
my $config_content = read_text( $config_path );
$config_content =~ s/^ \s* version \s* = \s* ['"]v\d+['"] /version = 'v$update'/xm;    # no 's'!
$config_content =~ s/^ \s* release \s* = \s* ['"]v\d+['"] /release = 'v$update'/xm;    # no 's'!
write_text($config_path, $config_content);
say "Updated version in docs config file.";
